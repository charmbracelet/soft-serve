...
func (b *Bubble) Help() []utils.HelpEntry {
	h := []utils.HelpEntry{}
	h = append(h, b.boxes[b.state].(utils.BubbleHelper).Help()...)
	if b.repo.Name() != "config" {
		h = append(h, utils.HelpEntry{Key: "R", Value: "readme"})
		h = append(h, utils.HelpEntry{Key: "F", Value: "files"})
		h = append(h, utils.HelpEntry{Key: "C", Value: "commits"})
		h = append(h, utils.HelpEntry{Key: "B", Value: "branches"})
	}
	return h
}
